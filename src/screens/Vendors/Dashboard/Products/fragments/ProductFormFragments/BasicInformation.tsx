...
export default function BasicInformation(){
    ...
    const onInitSelectCategory = () => {
        navigation.navigate(routes.categoryOptions as never,{
    ...
    return (
        <View flex={1}>
            <ScrollView flex={1} pt="2px">
                ...
                <CustomInput prefix={<CText color="gray.400" variant="body1">{decode(appContext.profileData?.currency?.currency_sym)}</CText>} keyboardType="number-pad" onChangeText={(val) => setFormValue(val,"sales_price")} my="8px" labelText={`Sales Price (${appContext.profileData?.currency?.currency_code})`} placeholder="Enter Sales Price (Optional)"  />
            ...
            <Box py="10px">
                <AppBtn onPress={() => true} gradient={true}>Continue</AppBtn>
            </Box>
        </View>
    )
}